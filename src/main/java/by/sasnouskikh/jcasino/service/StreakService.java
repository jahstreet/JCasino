...
/**
 * The class provides Service layer actions with streaks.
 *
 * @author Sasnouskikh Aliaksandr
 */
public class StreakService extends AbstractService {
    private static final Logger LOGGER = LogManager.getLogger(StreakService.class);
     ...
     */
    public StreakService(DAOHelper daoHelper) {
        super(daoHelper);
    }
     ...
     */
    public List<Streak> takePlayerStreaks(int id, String month) {
     ...
     */
    public List<Streak> takeStreakList(String month, boolean sortByTotal) {
        List<Streak> streakList   = null;
        String       monthPattern = (month != null ? month.trim() : EMPTY_STRING) + PERCENT;
        StreakDAO    streakDAO    = daoHelper.getStreakDAO();
        try {
        ...
        if (sortByTotal) {
            StreakService.sortByTotal(streakList, false);
        }
        return streakList;
    }

    /**
     * Calls DAO layer to save data about given {@link Streak} to database. Given {@link Streak} should be completed -
     * see {@link #isComplete(Streak)} and {@link #completeStreak(Streak)}.
     *
     ...
     */
    public Streak generateStreak(int playerId) {
        Streak    streak    = null;
        StreakDAO streakDAO = daoHelper.getStreakDAO();
        try {
     ...
     *
     * @param streaks {@link List} collection of {@link Streak} objects to be parsed
    ...
    private static class TotalComparator implements Comparator<Streak> {
        ...
        public int compare(Streak o1, Streak o2) {
            if (o1 == null || o1.getTotal() == null) {
                return -1;
            }
            if (o2 == null || o2.getTotal() == null) {
                return 1;
            }
