...
/**
 * The class provides Service layer actions with loans.
 *
...
public class LoanService extends AbstractService {
     ...
     */
    public LoanService(DAOHelper daoHelper) {
        super(daoHelper);
    }
     ...
     */
    public List<Loan> takePlayerLoans(int id, String month) {
        List<Loan> loanList       = null;
        String     acquirePattern = (month != null ? month.trim() : EMPTY_STRING) + PERCENT;
     ...
     */
    public List<Loan> takeLoanList(String acquire, String expire, boolean sortByRest, boolean filterNotPaid, boolean filterOverdued) {
        List<Loan> loanList       = null;
        String     acquirePattern = (acquire != null ? acquire.trim() : EMPTY_STRING) + PERCENT;
        String     expirePattern  = (expire != null ? expire.trim() : EMPTY_STRING) + PERCENT;
        LoanDAO    loanDAO        = daoHelper.getLoanDAO();
        try {
     ...
     */
    public boolean takeNewLoan(Player player, BigDecimal amount) {
        int        id            = player.getId();
        BigDecimal percent       = player.getAccount().getStatus().getLoanPercent();
        BigDecimal countedAmount = countLoan(amount, percent);
        PlayerDAO  playerDAO     = daoHelper.getPlayerDAO();
        LoanDAO    loanDAO       = daoHelper.getLoanDAO();
        try {
            daoHelper.beginTransaction();
            if (loanDAO.insertLoan(id, countedAmount, percent) != 0
     ...
     * @see PlayerDAO#changeBalance(int, BigDecimal, Transaction.TransactionType)
     */
    public boolean payLoan(Player player, BigDecimal amount) {
        int       id        = player.getId();
    ...
    private static void filterOverdued(List<Loan> list) {
        ...
        filterNotPaid(list);
        list.removeIf(l -> l.getExpire().isAfter(currentDate));
    }

    /**
     * Sorts given {@link List} collection of {@link Loan} objects by {@link Loan#rest} values in given order.
     *
     * @param list      {@link List} to be sorted
     * @param ascending marker of sorting order (true for ascending)
     * @see RestComparator
